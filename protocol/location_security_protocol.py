    ...
    def serialize(self, message_type, message_content):
        ...
        if self.message_type == 0x0101:
            ...
            content = struct.pack(
                ...
                week,                       # 参考周计数 (2字节)
                second,                     # 参考周计秒 (4字节)
                self.message_content.get('nav_system', 0x14),  # 导航系统标识 (1字节)
                self.message_content.get('nav_status', 0x00),  # 导航系统状态 (1字节)
                bytes.fromhex(self.message_content.get('signal_status', '00000000').zfill(8)[:8]),  # 导航信号状态 (4字节)
                bytes.fromhex(self.message_content.get('satellite_status', '0000000000000000').zfill(16)[:16]),  # 导航卫星状态 (8字节)
                b'\x00\x00\x00\x00\x00\x00\x00\x00'  # 保留字段 (8字节)
            )
        elif self.message_type == 0x0102:
            # 获取导航系统
            nav_system = self.message_content.get('nav_system', 0x14)
            ...
            content = struct.pack(
                '!H I B B B B 3s 3s',
                week if nav_system not in [0x31, 0x32, 0x33] else 0,  # 参考周计数 (2字节, GLONASS不需要)
                time_seconds,                     # 参考时间 (4字节)
                nav_system,                       # 导航系统标识 (1字节)
                int(self.message_content.get('verification_count', 0)),  # 电文验证信息数N (1字节)
                int(self.message_content.get('satellite_number', 0)),    # 卫星号 (1字节)
                int(self.message_content.get('message_type', 0x01)),     # 电文类型 (1字节)
                time_seconds.to_bytes(3, byteorder='big')[-3:],  # 电文参考时间 (3字节)
                bytes.fromhex('FFFFFF')  # 电文验证字 (3字节固定为FFFFFF)
            )
        else:
            # 其他消息类型的内容，留空
            content = b''
        ...
        # 注意：更新包长时，也要在添加 header 之后设置正确值
        header = struct.pack('!IBHH', self.FIXED_IDENTIFIER, self.FIXED_VERSION, package_length, self.message_type)

        # 更新组合顺序（需要重新组合，因为包长度改变）
        full_package = header + content
        if self.message_type == 0x0101:
            full_package += crc_bytes
